...
describe('ProductListComponent', () => {
  ...
  it('should load products from product service', () => {
    jest.spyOn(component['productService'], 'getProducts').mockReturnValue(of(mockProducts));
    component.ngOnInit();
    expect(component.products).toEqual(mockProducts);
  });

  it('should modal error when product service fails', () => {
    jest.spyOn(component['productService'], 'getProducts').mockReturnValue(throwError('Error'));
    component.ngOnInit();
    expect(component.modalOpen).toBe(true);
    expect(component.modalTitle).toBe('¡Ups algo salio mal!');
    expect(component.modalContent).toBe('No pudimos cargar los productos. Por favor, intenta nuevamente más tarde.');
  });
});

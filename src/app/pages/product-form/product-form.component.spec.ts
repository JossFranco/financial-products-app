...
describe('ProductFormComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [provideHttpClient(), { provide: ActivatedRoute, useValue: activatedRouteMock }]
    })
      .compileComponents();
  ...
  it('should mark all as touched and not submit if form is invalid', () => {
    ...
    const updateProductSpy = jest.spyOn(component['productService'], 'updateProduct');
  ...
  it('should load product info when is edit mode', () => {
    ...
    jest.spyOn(component['productService'], 'getProductById').mockReturnValue(of(mockProduct));
    component.ngOnInit();
  ...
  it('should show modal error if getProductById fails in loadProductIfEditMode', () => {
    activatedRouteMock.snapshot.paramMap.get.mockReturnValue('123');
    jest.spyOn(component['productService'], 'getProductById').mockReturnValue(throwError('Error'));
